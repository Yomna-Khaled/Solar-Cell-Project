...
# Render 404 page when record not found
  ...
  def allshifts
    if current_category.category=="Admin"
      ...
      @shifts = Shift.all
      @shifts = Shift.where("end_shift_date IS NOT NULL").paginate(:page => params[:page], :per_page => 6).order(id: :desc)
    else
...
if current_category.category=="Shift Manager" 
  ...
  private
    ...
    def end_shift_params
       ...
       params[:shift][:end_shift_Date] =Time.zone.now.strftime('%d %b %Y')
       params.require(:shift).permit(:end_shift_date,:end_shift_time)
    end 
